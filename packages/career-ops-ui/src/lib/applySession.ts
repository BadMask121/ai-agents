...
/**
 * Apply a set of per-index field patches to an existing payload. Used by the
 * iterate flow (chat-driven edits) — the agent returns a JSON block with
 * only the changed fields, this helper merges them onto the existing payload
 * in a single atomic read-modify-write. Unknown indices are ignored rather
 * than throwing so a stale agent response can't corrupt the session.
 *
 * Each patched field is marked `source: "user"` since the user asked for
 * the change via chat, even though the text was generated by the agent.
 */
export async function patchPayloadFields(
  id: string,
  patches: Array<{ index: number; answer: string }>,
): Promise<ApplySession> {
  return mutate(id, (s) => {
    if (!s.payload) return s;
    const newFields = [...s.payload.fields];
    for (const patch of patches) {
      if (
        Number.isInteger(patch.index) &&
        patch.index >= 0 &&
        patch.index < newFields.length &&
        typeof patch.answer === "string"
      ) {
        newFields[patch.index] = {
          ...newFields[patch.index],
          answer: patch.answer,
          source: "user",
        };
